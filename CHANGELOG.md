...
* Fixes the issue where the client might crash when invoking `updateRemoteNotificationDeviceToken:error:` with a `nil` token. [IOS-3301]
* Fixes an issue where the conversation's unread and total message count could be off, which would also result in a wrong unread state of the conversation. [IOS-3281]

## 0.26.12

#### Bug Fixes

* Re-releasing the previous build with bit-code enabled.

## 0.26.11

#### Bug Fixes

* Fixes an issue where the client would ignore the `LYRClientOptions`, the configuration endpoints specifically.
